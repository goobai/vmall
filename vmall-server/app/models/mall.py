...
class ProductSpu(db.Model):
    ...
    def product_update(self, **kwargs):
        ...
        filter = ProductSpu.query.filter_by(kwargs).all()
        product = ProductSpu(brand_id=self.brand_id, cid=self.cid, shop_id=self.shop_id, title=self.title,
                             name=self.name, create_time=self.create_time, modify_time=datetime.now(),
                             status=self.status)
        db.session.commit()
...
class ProductSku(db.Model):
    ...
    @staticmethod
    def update_sku(sku_id, **kwargs):
        sku = ProductSku.query.filter_by(id=sku_id).first()
        if sku:
            sku.name = kwargs.get('name')
            sku.price = kwargs.get('price')
            sku.stock = kwargs.get('stock')
            sku.modify_time = kwargs.get('modify_time')
            sku.status = kwargs.get('status')
            db.session.commit()
            return True
        else:
            return False
...
class Order(db.Model):
    ...
    order_status = db.Column(db.SmallInteger,
                             default=0)  # 订单状态状态 0:生成订单，待付款 ；1：付款完成，待发货；2：发货完成，物流中，待确认收货 ；3：确认收货，待评价 4：评价完商品，订单完成
    status = db.Column(db.SmallInteger, default=0)  # 状态
...
class OrderShip(db.Model):
    ...
    id = db.Column(db.BIGINT, primary_key=True)
    order_id = db.Column(db.BIGINT, index=True)  # 订单编号
    ship_id = db.Column(db.BIGINT, index=True)  # 物流编号
